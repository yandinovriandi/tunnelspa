<x-app-layout>
    <x-slot name="header">
       <div class="flex justify-between">
           ...
           <Link href="{{route('tunnels.create')}}" class="px-2 py-1.5 bg-indigo-500 text-indigo-100 font-semibold hover:bg-indigo-700 hover:text-white rounded-md">New Tunnel</Link>
       </div>
    </x-slot>
...
{{--                    <x-splade-table :for="$tunnels" striped>--}}
{{--                        <x-slot name="head">--}}
{{--                            <thead>--}}
{{--                            <tr>--}}
{{--                                <th>#</th>--}}
{{--                                @foreach($tunnels->columns() as $column)--}}
{{--                                    <th>{{ $column->label }}</th>--}}
{{--                                @endforeach--}}
{{--                            </tr>--}}
{{--                            </thead>--}}
...
{{--                        <x-slot name="body">--}}
{{--                            <tbody>--}}
{{--                            @foreach($tunnels->resource as $tunnel)--}}
{{--                                <tr>--}}
...
{{--                                    <td>{{$tunnel->host}}</td>--}}
{{--                                    <td>{{$tunnel->server}}</td>--}}
{{--                                    <td>{{ $tunnel->auto_renew ? 'Ya' : 'Tidak' }}</td>--}}
{{--                                    <td>{{ $tunnel->status}}</td>--}}
{{--                                    <td>{{ $tunnel->expired }}</td>--}}
{{--                                    <td>--}}
...
{{--                                              cancel-button="No!"--}}
{{--                                              href="{{route('server.destroy',$tunnel)}}" method="DELETE">--}}
...
{{--                    </x-splade-table>--}}
                    <x-splade-table :for="$tunnels">
                        ...
                        @if ($tunnel->auto_renew == 'ya')
                           <span class="bg-green-100 capitalize hover:cursor-pointer hover:text-green-700 font-semibold text-green-500 rounded-md px-4 py-0.5">{{$tunnel->auto_renew}}</span>
                        @elseif($tunnel->auto_renew == 0)
                            <span class="bg-yellow-100 capitalize hover:cursor-pointer hover:text-yellow-700 font-semibold text-yellow-500 rounded-md px-4 py-0.5">Tidak</span>
                        @else
                            <span class="bg-yellow-100 capitalize hover:cursor-pointer hover:text-yellow-700 font-semibold text-yellow-500 rounded-md px-4 py-0.5">{{$tunnel->auto_renew}}</span>
                        @endif
                        @endcell
                        @cell('status',$tunnel)
                        @if ($tunnel->status == 'aktif')
                            <span class="bg-green-100 capitalize hover:cursor-pointer hover:text-green-700 font-semibold text-green-500 rounded-md px-4 py-0.5">{{$tunnel->status}}</span>
                        @else
                            <span class="bg-yellow-100 capitalize hover:cursor-pointer hover:text-yellow-700 font-semibold text-yellow-500 rounded-md px-4 py-0.5">{{$tunnel->status}}</span>
                        @endif
                        @endcell
                        @cell('actions',$tunnel)
                          <div class="flex items-center justify-center gap-x-1">
                              <Link method="PUT" href="{{route('tunnels.renew',$tunnel)}}" class="text-blue-500 hover:text-blue-700">
